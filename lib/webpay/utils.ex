defmodule Webpay.Utils do
  @moduledoc false

  @currency_list ~w(jpy)

  @doc """
  Get currency from atom.
  """
  def fetch_currency!(currency) when is_atom(currency) do
    name = to_string(currency)
    if name in @currency_list do
      currency
    else
      raise "#{currency} is not supported yet in Webpay"
    end
  end
